...
class DataArranger(GenericDataArranger):
    ...
    def get_feature_list():
        feature_list = ['vggish', 'mfcc', 'bert']
        return feature_list

    def partition_range_fn(self):
        partition_range = {
            'train': [np.arange(0, 68), np.arange(68, 136), np.arange(136, 204), np.arange(204, 272), np.arange(272, 341)],
            'validate': [np.arange(341, 412)],
            'test': [],
            'extra': [np.arange(412, 564)]}
