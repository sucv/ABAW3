...
class GenericTrainer(object):
    ...
    def init_optimizer_and_scheduler(self, epoch=0):
        ...
        reduce_on_plateau_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min',
                                                                    patience=self.patience, factor=self.factor)
        if epoch > self.min_epoch:
            self.scheduler = GradualWarmupScheduler(self.optimizer, total_epoch=self.min_epoch, after_scheduler=reduce_on_plateau_scheduler)
        else:
